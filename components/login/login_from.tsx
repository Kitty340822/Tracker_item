...
const LoginForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // เปลี่ยนจาก 'users' เป็น 'credentials'
            const result = await signIn('credentials', {
                email,
                password,
                redirect: false, // เพิ่ม redirect: false
            });

            console.log('SignIn result:', result); // สำหรับ debug

            if (result?.error) {
                setError('อีเมลหรือรหัสผ่านไม่ถูกต้อง');
            } else if (result?.ok) {
                router.push('/admin'); 
                router.refresh(); // เพิ่ม refresh เพื่ออัปเดต session
            } else {
                setError('เกิดข้อผิดพลาดในการเข้าสู่ระบบ');
            }
        } catch (err) {
            setError('เกิดข้อผิดพลาดในการเชื่อมต่อ');
            console.error('Login error:', err); // เพิ่ม logging
        } finally {
            setLoading(false);
        }
    };
