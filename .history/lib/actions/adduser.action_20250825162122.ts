...
export const getSignupList = async (): Promise<ISignup[]> => {
...
export const getSignupListBd = async (id: string): Promise<ISignup[]> => {
  const res = await fetch(`http://localhost:3000/api/signup/${id}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      apikey: "1234567890",
