...
export const AddUserDialog = ({
  ...
  const handlePermissionCheck = (perm: string) => {
    const currentPerms = usrForm.getValues("role_permission");
    const existsIdx = currentPerms((p) => p === perm);
    ...
    if (existsIdx >= 0) {
      ...
      usrForm.setValue("role_permission", [...currentPerms, perm]);
    }
  };
  ...
  const handleSubmit = async (values: z.infer<typeof CreateUserSchema>) => {
    try {
      ...
      if (success) {
        successCallback();
        setOpen(false);
        usrForm.reset(INIT_FORM); // Reset form after successful submission
        toast({
